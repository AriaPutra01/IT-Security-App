...
export function IsoPage() {
  ...
  const AddSubmit = async (data) => {
    try {
      await addIso(data); // tambah data ke API
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
  ...
  const EditSubmit = async (data) => {
    try {
      await updateIso(data.ID, data); // edit data ke API
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await deleteIso(id); // hapus data di API
  ...
  const handleBulkDelete = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await Promise.all(selectedIds.map((id) => deleteIso(id))); // hapus data di API
