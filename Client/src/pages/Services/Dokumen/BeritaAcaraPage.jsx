...
export function BeritaAcaraPage() {
  ...
  const AddSubmit = async (data) => {
    try {
      await addBeritaAcara(data); // tambah data ke API
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
  ...
  const EditSubmit = async (data) => {
    try {
      await updateBeritaAcara(data.ID, data); // edit data ke API
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await deleteBeritaAcara(id); // hapus data di API
  ...
  const handleBulkDelete = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await Promise.all(selectedIds.map((id) => deleteBeritaAcara(id))); // hapus data di API
