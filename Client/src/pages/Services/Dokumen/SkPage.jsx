...
export function SkPage() {
  ...
  const AddSubmit = async (data) => {
    try {
      await addSk(data); // tambah data ke API
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
  ...
  const EditSubmit = async (data) => {
    try {
      await updateSk(data.ID, data); // edit data ke API
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await deleteSk(id); // hapus data di API
  ...
  const handleBulkDelete = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await Promise.all(selectedIds.map((id) => deleteSk(id))); // hapus data di API
