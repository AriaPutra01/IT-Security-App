...
export function MemoPage() {
  ...
  const EditSubmit = async (data) => {
    try {
      await updateMemo(data.ID, data); // edit data ke API
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await deleteMemo(id); // hapus data di API
  ...
  const handleBulkDelete = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await Promise.all(selectedIds.map((id) => deleteMemo(id))); // hapus data di API
