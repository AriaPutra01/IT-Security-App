...
func CreateArsip(c *gin.Context) {
	...
	if err := initializers.DB.Create(&arsip); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": "Failed to create arsip"})
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "Arsip created successfully", "data": arsip})
}
...
func UpdateArsip(c *gin.Context) {
	...
	if err := initializers.DB.Update(id, &arsip); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": "Failed to update arsip"})
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "Arsip updated successfully", "data": arsip})
}

// Fungsi untuk menghapus arsip
func DeleteArsip(c *gin.Context) {
	id := c.Param("id")
	if err := initializers.DB.Delete(id); err != nil {
